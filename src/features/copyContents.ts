...
class FileProcessor {
    ...
    async processFile(fileUri: vscode.Uri): Promise<FileContent | null> {
        ...
        try {
            ...
            if (
                ...
                this.logger.log(
                    `Skipping large file: ${fileUri.fsPath} (${(
                        stat.size /
                        1024 /
                        1024
                    ).toFixed(2)}MB)`,
            ...
            if (
                this.config.ignoreBinaryFiles &&
                this.isBinaryFile(fileUri, fileBytes)
            ) {
    ...
    async processFiles(
        ...
        const optimalChunkSize = Math.min(
            Math.max(4, Math.floor(fileUris.length / 10)),
            32,
        )

        for (let i = 0; i < fileUris.length; i += optimalChunkSize) {
            const chunk = fileUris.slice(
                i,
                Math.min(i + optimalChunkSize, fileUris.length),
            )
            ...
            progressReporter.report(
                `Processing ${processed}/${fileUris.length}...`,
            )
        }
            ...
            const directoryPromises = entries
                ...
                .map(async ([name, type]) => {
                    ...
                    if (type === vscode.FileType.File) {
                        ...
                        return collectFileUrisOptimized(
                            childUri,
                            patterns,
                            workspaceRootUri,
                            logger,
...
function formatOutput(
    files: FileContent[],
    config: Pick<CopyConfig, "includeStats" | "addPrompt" | "defaultPrompt">,
    prompt?: string,
): {
    output: string
    sizeBytes: number
...
export async function copyAllContents(
    ...
    const config: CopyConfig = {
        excludePatterns: getConfig("codeBridge", "exclude", [
            "**/node_modules",
            "**/.git",
        ]),
    ...
    try {
        ...
        const allFileUris = [...fileSet.values()].sort((a, b) =>
        ...
        const { output: finalContent, sizeBytes } = formatOutput(
            fileContents,
            config,
            prompt,
        )
        ...
        if (
            config.lineWarningLimit > 0 &&
            finalLineCount > config.lineWarningLimit
        ) {
        ...
        if (!config.disableSuccessNotifications) {
            ...
            const message = `Copied ${
                fileContents.length
            } files | ${finalLineCount.toLocaleString()} lines | ${sizeMB}`
