...
async function buildTree(
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        const connector = isLast
            ? options.connectors.last
            : options.connectors.branch
        const extension = isLast
            ? options.connectors.extension
            : options.connectors.line
        ...
        if (type === vscode.FileType.Directory) {
            ...
            result.push(
                `${prefix}${connector}${options.directoryIcon}${name}${options.directorySuffix}`,
            )
...
export async function copyProjectTree(
    ...
    const treeStyle = getConfig<TreeStyle>(
        "codeBridge",
        "tree.style",
        "classic",
    )
